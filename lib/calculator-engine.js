...
const engine = (priorEngine = INITIAL_ENGINE, action) => {
  ...
  if (!hasError(priorEngine) && action.type === 'BINARY_OPERATION') {
...
const Operation = {
  add: () => multiDispatch(minArgs(2), executeBinaryOperation(Operations.Add)),
  divide: () => multiDispatch(minArgs(2), isNonZeroRegister(0), executeBinaryOperation(Operations.Divide)),
  multiply: () => multiDispatch(minArgs(2), executeBinaryOperation(Operations.Multiply)),
  subtract: () => multiDispatch(minArgs(2), executeBinaryOperation(Operations.Subtract)),
  sin: () => multiDispatch(minArgs(1), executeUnaryOperation(UnaryOperations.Sin)),
};
