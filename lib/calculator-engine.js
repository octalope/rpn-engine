...
const hasError = (priorEngine) => Object.prototype.hasOwnProperty.call(priorEngine, 'error');
...
const isNonZeroArg = (reg) => {
  return {
    type: 'INPUT_VALIDATION',
    payload: {
      validationType: 'NON_ZERO_ARG',
      register: reg
    }
  };
};

const isNonNegativeArg = (reg) => {
  return {
    type: 'INPUT_VALIDATION',
    payload: {
      validationType: 'NON_NEGATIVE_ARG',
      register: reg
    }
  };
};
...
const processInputValidation = (priorEngine, action) => {
  ...
  switch (action.payload.validationType) {
    case 'MIN_ARGS':
      return checkMinArgs();
    case 'NON_ZERO_ARG':
      return checkNonZeroArg();
    case 'NON_NEGATIVE_ARG':
      return checkNonNegativeArg();
  }
  return priorEngine;
};
...
const processOperation = (priorEngine, action) => {
  ...
  switch (action.payload) {
    ...
    case Operations.Sqrt:
      return execOne(priorEngine, (x) => Math.sqrt(x));
  }
};
...
const Operation = {
  ...
  divide: () => multiDispatch(minArgs(2), isNonZeroArg(0), executeOperation(Operations.Divide)),
  ...
  sqrt: () => multiDispatch(minArgs(1), isNonNegativeArg(0), executeOperation(Operations.Sqrt)),
};
